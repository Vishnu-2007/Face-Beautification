...
function initDashboardLogic() {
    ...
    if (confirmPassUpdateBtn) {
        confirmPassUpdateBtn.onclick = async () => {
            ...
            try {
                const res = await fetch('https://track-villain-devourer.ngrok-free.dev/api/auth/update-password', {
                    method: 'POST',
                    headers: { 
...
async function fetchUserHistory() {
    ...
    try {
        const res = await fetch('https://track-villain-devourer.ngrok-free.dev/api/user/history', {
        ...
        if (history.length > 0) {
            historyList.innerHTML = history.map(item => `
                <div class="history-item tilt-3d">
                    <img src="https://track-villain-devourer.ngrok-free.dev/uploads/${item.beautified_image}" alt="Beautified">
